...
class WebLogParserTest < Minitest::Test
  ...
  def test_strict_parse_line
    testcase = read_testcase('line.log')
    assert_equal(@parser.parse(testcase), @parser.parse!(testcase))
  end
  
  def test_strict_parse_line_should_raise_on_invalid_format
    error = assert_raises(WebLogParser::ParseError) { results = @parser.parse!('foobar') }
